...
const CartPage = ({ cart, removeFromCart }) => {
  ...
  // Mettre à jour le panier à chaque modification de la prop cart
  useEffect(() => {
    setUpdatedCart(cart);
  }, [cart]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-6">
                {updatedCart.map((item) => (
                  <div key={item.id} className="flex items-center justify-between border-b pb-4">
                    ...
                    <div className="flex items-center space-x-4">
                      <button className="p-2 bg-gray-200 rounded-full" onClick={() => handleDecreaseQuantity(item.id)} aria-label="Diminuer la quantité">
                        <AiOutlineMinus size={20} />
                      </button>
                      <span className="text-lg font-semibold">{item.quantity}</span>
                      <button className="p-2 bg-gray-200 rounded-full" onClick={() => handleIncreaseQuantity(item.id)} aria-label="Augmenter la quantité">
                        <AiOutlinePlus size={20} />
                      </button>
                      <button className="ml-4 text-red-500" onClick={() => removeFromCart(item)} aria-label="Supprimer l'article">
                        <AiFillDelete size={20} />
                      </button>
                    </div>
                    <p className="text-lg font-semibold text-gray-800">{formatPrice(item.quantity * parseFloat(item.originalPrice))} TND</p>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="bg-white shadow-lg rounded-lg p-6">
            ...
            <div className="space-y-4">
              ...
              <p className="text-lg font-semibold text-gray-800">Total TTC : {formatPrice(totalWithStamp)} TND</p>
            </div>
            <div className="bg-gray-100 p-4 rounded-lg mt-6">
              <p className="text-sm text-gray-600">Dépensez <span className="font-bold">{formatPrice(145000)} TND</span> pour obtenir une livraison gratuite !</p>
            </div>
            <div className="mt-6">
             
                <button className="w-full bg-green-600 text-white py-3 rounded-md hover:bg-green-700" onClick={handleOrderClick} aria-label="Commander">
                  Commander
                </button>
